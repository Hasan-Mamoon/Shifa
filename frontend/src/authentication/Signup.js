...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.email || !formData.password || !formData.name || !formData.image || !formData.address.line1 || !formData.address.line2) {
      setError("Please fill all required fields.");
      return;
    }

    if (userType === "doctor" && (!formData.speciality || !formData.degree || !formData.experience || !formData.about || !formData.fees)) {
    ...
    try {
      const url = userType === "doctor" ? "http://localhost:3080/doctor/add-doctor" : "http://localhost:3080/patient/add-patient";
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
      <h2 className="text-2xl font-bold mb-4 text-center">{userType === "doctor" ? "Doctor" : "Patient"} Signup</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <input type="text" name="address.line1" placeholder="Address Line 1" value={formData.address.line1} onChange={handleChange} required className="w-full p-2 border rounded" />
        <input type="text" name="address.line2" placeholder="Address Line 2" value={formData.address.line2} onChange={handleChange} required className="w-full p-2 border rounded" />
        ...
        {userType === "doctor" && (
          <>
            <input type="text" name="speciality" placeholder="Speciality" value={formData.speciality} onChange={handleChange} required className="w-full p-2 border rounded" />
            <input type="text" name="experience" placeholder="Experience (years)" value={formData.experience} onChange={handleChange} required className="w-full p-2 border rounded" />
            <textarea name="about" placeholder="About Yourself" value={formData.about} onChange={handleChange} required className="w-full p-2 border rounded" />
            <input type="number" name="fees" placeholder="Consultation Fees" value={formData.fees} onChange={handleChange} required className="w-full p-2 border rounded" />
            <input type="file" name="degree" accept=".pdf,.jpg,.png" onChange={handleFileChange} required className="w-full p-2 border rounded" />
          </>
        )}

        <input type="file" name="image" accept="image/*" onChange={handleFileChange} required className="w-full p-2 border rounded" />
