...
//       console.log("Updated Data: ", { ...updatedData, address, image });
...
const Profile = () => {
  ...
  //     // Log FormData for debugging (this can be logged to check the content being sent)
  ...
  //       setSuccessMessage("Profile updated successfully.");
  //       toggleEditMode(); // Switch back to non-edit mode
  //       fetchDoctorData(); // Refresh the data to reflect changes
  //     }
  //   } catch (error) {
  //     setError("Failed to save doctor data.");
  //   }
  ...
  //     // Ensure that address is appended properly
  //     if (address) {
  //       Object.keys(address).forEach((key) => {
  //         if (address[key]) {
  //           formData.append(`address[${key}]`, address[key]);
  ...
  const handleSave = async () => {
    try {
      ...
      // Construct updated data for sending
      const updatedDoctorData = { ...updatedData, address };
  
      // If there's an image (e.g., file path or image URL), add it to the object
      if (image) {
        updatedDoctorData.image = image;
      }
  
      // Log the data being sent
      console.log("Updated Data (JSON):", updatedDoctorData);
