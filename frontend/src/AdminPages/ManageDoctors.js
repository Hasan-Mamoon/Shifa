...
const ManageDoctor = () => {
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      await axios.put(`${process.env.REACT_APP_SERVER_URL}/admin/update-doctor-status/${id}`, { status });
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {doctors.map((doctor) => (
          ...
          >
            <img src={doctor.image} alt={doctor.name} className="w-32 h-32 rounded-full object-cover mx-auto" />
      ...
      {isModalOpen && (
        <Modal doctor={selectedDoctor} onClose={() => setIsModalOpen(false)} />
      )}
    </div>
  );
};
