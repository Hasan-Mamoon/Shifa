...
const MyProfile = () => {
    const [userData,setUserData]=useState({
        ...
        email:'f219418@cfd.nu.edu.pk',
        phone:'34453343423',
        address:{
line1:"faislabad city",
    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
            ...
            {
        isEdit?<input className='bg-gray-100 max-w-52' type="text" value={userData.phone} onChange={e=>setUserData(prev=>({...prev,name:e.target.value}))}/>: <p className='text-blue-400'>{userData.phone}</p>
      }
      <p className='font-medium'>Address:</p>
      {
        isEdit?<p><input className='bg-gray-50' onChange={(e)=>setUserData(prev=>({...prev,address:{...prev.address,line1:e.target.value}}))} value={userData.address.line1} type="text" /><br /><input className='bg-gray-50' onChange={(e)=>setUserData(prev=>({...prev,address:{...prev.address,line2:e.target.value}}))} value={userData.address.line2} type="text" /></p>: <p className='text-gray-500'>{userData.address.line1}bridge{userData.address.line2}</p>
      }
        </div>
      </div>
      <div>
