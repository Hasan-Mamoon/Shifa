...
const Calendar = () => {
  ...
  const fetchEventsForMonth = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/calendar?start=${startDate}&end=${endDate}`);
  ...
  const handleDeleteEvent = async (id) => {
    try {
      const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/calendar/${id}`, { method: "DELETE" });
      if (res.ok) {
      ...
      days.push(
        <div
          ...
          className={`p-3 text-center rounded-full cursor-pointer transition ${
            ...
            isHighlighted ? "bg-blue-200 text-black" : "bg-gray-100 hover:bg-blue-200"
          }`}
      ...
      <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-4xl flex">
        ...
        <div className="w-2/3">
          <div className="flex justify-between items-center mb-4">
            <button onClick={() => changeMonth(-1)} className="text-gray-600 text-lg hover:text-blue-500">
              ◀
            </button>
